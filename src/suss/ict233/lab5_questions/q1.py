...
class Question1B(CodingProblem):
    ...
    def produce_expected(phone_data_file):
        ...
        df_inner = exec_globals['produce_expected'](expected_file)
        return df_inner
    ...
    def check(self, *args):
        for test in self._test_cases:
            expected = Question1B.produce_expected(test[0])
            ...
            expected_df_inner = Question1B.produce_expected(test[1])
...
class Question1C(CodingProblem):
    ...
    def produce_expected(phone_data_file):
        expected_file = os.path.dirname(os.path.abspath(__file__)) + "/" + phone_data_file
        produce_expected_code = x.get_produce_expected("lab5", "q1", "q1c")
        exec_globals = {}
        exec(produce_expected_code, exec_globals)
        df_right = exec_globals['produce_expected'](expected_file)
        return df_right
    ...
    def check(self, *args):
        for test in self._test_cases:
            expected = Question1C.produce_expected(test[0])
            ...
            expected_df_right = Question1C.produce_expected(test[1])
