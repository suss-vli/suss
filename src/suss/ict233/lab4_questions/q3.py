...
class Question3(CodingProblem):
    ...
    def produce_expected(employment_rate_file, labour_rate_file):
        combine_both = Question2.produce_expected(employment_rate_file, labour_rate_file)
        produce_expected_code = x.get_produce_expected("lab4", "q3", "q3")
        exec_globals = {}
        exec(produce_expected_code, exec_globals)
        expected_combine_both = exec_globals['produce_expected'](combine_both)
    ...
    def check(self, *args):
        for test in self._test_cases:
            ...
            expected_combine_both = Question3.produce_expected(test[0], test[1])
            print("type of expected_combine_both", type(expected_combine_both))
            print("type of actual_combine_both", type(actual_combine_both))
            ...
            expected_test_combine_both = Question3.produce_expected(test[2], test[3])
