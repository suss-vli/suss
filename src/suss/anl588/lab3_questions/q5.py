...
class Question5(EqualityCheckProblem):
    ...
    def produce_expected(data1 = 0.3, data2 = 101):
        df = Question3A._expected
        produce_expected_code = x.get_produce_expected("lab3", "q5", "q5")
        exec_globals = {}
        exec(produce_expected_code, exec_globals)
        X_train, X_test, y_train, y_test = exec_globals['produce_expected'](df, data1, data2)
