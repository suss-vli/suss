...
from learntools.vli.encryption import decrypt

def get_produce_expected(lab, main_qns, qns):
    config_data = query_config(".config.yml")
    file_path = config_data.get('suss_path')

    produce_expected_file = os.path.join(
        file_path,
        f"{lab}_questions", 
        f"{main_qns}", 
        f"{qns}", 
    ...
    with open(produce_expected_file, 'r') as produce_expected_content:
        encrypted_code = produce_expected_content.read()
    produce_expected_code = decrypt(encrypted_code)
    
    return produce_expected_code