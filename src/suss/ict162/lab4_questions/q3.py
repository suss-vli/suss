...
class Question3B (FunctionProblem):
    ...
    today = datetime.now()
    _test_cases = [
        (['PP1', 'John', 1960], ['PP2', 'Jane', 2000], ['SQ1', 'LA', 2000, datetime(2026, 12, 25, 4, 15)], ['SQ52', 'KL', 1000, datetime(2024, 4, 3, 12, 35)], ['SQ99', 'NY', 2500, datetime(2026, 10, 15, 20, 10)], 'Singapore Airline', 'SUSS', ['name', 'ppNo', 'yearBorn'], ['departureDate', 'destination', 'fare', 'flightNo'], ['_IndividualBooking__discount', 'bookingDate', 'bookingId', 'flight', 'getNextBookingId', 'passenger', 'ticketPrice'], ['addBooking', 'changeBooking', 'deleteBooking', 'searchBooking'], f"""Singapore Airline
No bookings yet
Singapore Airline
...
Flight: SQ1 Destination: LA Departure Date: 25/12/2026 04:15 Fare: $2000.00
Duplicate booking error
Singapore Airline
Booking id: 1 Booking Date: {datetime(today.year, today.month, today.day).strftime("%d %B %Y")}
Ticket Price: $2000.0
Passport No: PP1 Name: John Year Born:1960
Flight: SQ99 Destination: NY Departure Date: 15/10/2026 20:10 Fare: $2500.00
...
Non existing booking error\n""")
    ...
    def check_testbook(self, fn):
        for test in self._test_cases:           
            ...
            if actual is None:
                x.justfail("None", f"{self.__var}() is {actual}. Please attempt the question and run the question again.")
            else:
            ...
            x.grading_check_setter("`fl1.flightNo = 'MY2'`", 'MY2', fl1, "flightNo", fl1._Flight__flightNo, "@flightNo.setter")
            x.grading_check_setter("`fl1.departureDate = datetime(2023, 5, 13, 10, 35)`", datetime(2023, 5, 13, 10, 35), fl1, "departureDate", fl1._Flight__departureDate, "@departureDate.setter")
            # x.grading_check_setter("`b1.flight = Flight('MY2', 'NY', 2500, datetime(2022, 10, 21, 2, 15))`", Flight('MY2', 'NY', 2500, datetime(2022, 10, 21, 2, 15)), b1, "flight", b1.__flight, "@flight.setter")
